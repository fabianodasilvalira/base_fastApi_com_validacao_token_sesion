...
async def create(cliente: ClienteCreate, db_session: AsyncSession = Depends(get_db_session)):
    return await create_cliente(db_session, cliente)


@router.get("/{cliente_id}", response_model=ClienteOut)
async def read(cliente_id: str, db_session: AsyncSession = Depends(get_db_session)):
...
async def read_multiple(skip: int = 0, limit: int = 10, db_session: AsyncSession = Depends(get_db_session)):
    return await get_clientes(db_session, skip, limit)


@router.put("/{cliente_id}", response_model=ClienteOut)
async def update(cliente_id: str, cliente: ClienteUpdate, db_session: AsyncSession = Depends(get_db_session)):
...
async def delete(cliente_id: str, db_session: AsyncSession = Depends(get_db_session)):
